...
本文描述当前仓库中 `remote-dpd` 的实际实现。代码和自动化测试是行为的最终依据；未实现能力会明确标为后续阶段。

## [AGENT] 已批准的最终阶段目标

整体重构的前三个阶段已完成。剩余阶段 4 将在当前 Python API、运行存储和文件命令服务之上增加本机网页控制台，提供受限波形浏览、动态设备 schema 表单、PA 系数编辑、分步/自动控制、实时状态与指标、临时任务浏览和最终 MAT 下载。

网页默认只监听 loopback，不提供登录、多用户、多任务并行或公网部署。本节描述已批准但尚未实现的最后阶段；本文其余部分只描述当前代码。
...
- 对外常驻入口是版本化 inbox/outbox MAT 命令服务。
...
唯一闭环实现位于 `ClosedLoopController`。文件入口只解析命令、管理 controller 生命周期、同步存储并投递状态；设备、预处理和算法均不知道 MAT 文件或 watcher。
...
| `cli.py` | 服务参数、RunStore/FileCommandService 生命周期和常驻循环 |
...
命令 schema 当前为 1，公共变量是 `schema_version/command_id/action`，按动作可带 `x/config_json`。动作包括 `load/configure/power_tune/calibrate/step/run/stop/reset/export`。
...
通用 `load_mat()` 使用 SciPy 加载 v5/v6/v7，并递归解包 MATLAB struct、structured array 和 object array。仅当 SciPy 抛出 `NotImplementedError` 时尝试可选 h5py；该回退只支持有限顶层 dataset，不是完整 MATLAB v7.3 struct 解码器。
