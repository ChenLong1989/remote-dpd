...
- LS 采用朴素 Tikhonov 正则的正规方程（`(A^H A + ridge*I) c = A^H z`，即 `min ||A c - z||^2 + ridge*||c||^2`，直接惩罚原始系数模值），`numpy.linalg.solve` 求解；Gram 与右端分块累积（每块约 800 万基样点）以限制长波形的峰值内存。全零列直接赋零系数，病态矩阵回退 `lstsq`。不用列归一化的相对正则的原因：GMP 高阶列能量极小（真机数据上 p=9 列能量比 p=1 列低约 6 个数量级），朴素 λ 会按列能量自动加权压制高阶病态方向——正是这些方向的系数爆炸（|c|max 数百）曾把拟合 Jacobian 在波形峰样点放大成梯度尖峰，导致真机 `candidate_peak_exceeded`（2026-09-03）。默认 `ridge=1e-5` 在真机数据上保留 GMP 残差收益约 4.5 dB（-27.4 → -31.9 dB），同时把种子步梯度尖峰比从约 60 压到约 13。
- `g` 是 `||F(y) - x||^2` 相对复参数化 `y` 的精确实梯度方向。非全纯映射必须包含共轭项 `c_k*(dBasis/dy_j*)*conj(e)`：只取 `A^H e` 或误用 `(A+B)^H e` 都会使方向倾斜，深度压缩下足以导致停滞或发散。方向公式（对齐与交叉路径）均由中心有限差分验证（真机尺度：代理目标梯度差分 8.4e-06，伴随恒等式 1.3e-09）。
...
| `ridge` | `1e-5` | `[1e-12, 1e-2]` 内有限实数；朴素 Tikhonov λ，直接惩罚原始系数模值（列不归一化、不乘 N，量纲随参考 RMS 条件化固定） |
