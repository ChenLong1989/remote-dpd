...
在当前 `10×20 MHz @ 491.52 MS/s` 默认 waveform、reference RMS `-15 dBFS`、物理目标 `-15 dBm` 和 Web quick-start 的 `forward_model_ilc`、`mu=1.0`、15 次 ILC 下，功率调节锁定约 `0.7 dB/-15.16 dBm`。以左右最外侧 20 MHz 载波为 main、紧邻外侧 20 MHz 为 adjacent，第 0 轮分别为 `-28.19/-29.29 dBc`，第 15 轮为 `-41.19/-42.48 dBc`；NMSE 从约 `-24.84 dB` 单调改善到 `-38.01 dB`，最终数字峰值约 `0.519`。同场景下 `basic_ilc`（`mu=0.35`）在第 11 轮达到约 `-36.94 dB` 后开始发散，第 15 轮退化到约 `-35.85 dB` 且数字峰值升至约 `0.710`；PA 三阶项使静态传输斜率在数字峰值约 `0.64` 处过零，任何 ILC 算法在该 waveform 下的可达下限约 `-38.5 dB`。该结论只保证当前默认 waveform 和预设，不保证用户任意系数组合收敛。
