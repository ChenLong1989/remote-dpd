...
本文描述 `remote_dpd/power_control.py` 和 `remote_dpd/controller.py` 的当前实现。控制器已经可以通过 Python API 在 `SimulatedRFBench` 上完成分步或自动闭环，但尚未接入默认 CLI、文件协议或网页入口。
...
`ControllerSnapshot` 提供一致的状态、连接/配置/发射标记、当前操作、停止标记、最大/当前轮次、固定增益、锁定衰减、最近一次实际监控功率、参考安全报告、全部已提交记录、功率调节轨迹和最后结构化错误。最近功率独立于已提交记录，因此功率越界或后续抓取失败时仍保留触发本轮行为的读数。`y`/`z` 属性只引用最新完整记录。
